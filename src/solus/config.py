...
class Config:
    ...
    workflows_dir: Path = Path("~/.config/solus/workflows.d").expanduser().resolve()
    modules_dir: Path = Path("~/.config/solus/modules.d").expanduser().resolve()
    triggers_dir: Path = Path("~/.config/solus/triggers.d").expanduser().resolve()


DEFAULT_CONFIG_PATH = Path("~/.config/solus/config.toml")
DEFAULT_CACHE_DIR = "~/.local/share/solus"
DEFAULT_WORKFLOWS_DIR = Path("~/.config/solus/workflows.d")
DEFAULT_MODULES_DIR = Path("~/.config/solus/modules.d")
DEFAULT_TRIGGERS_DIR = Path("~/.config/solus/triggers.d")
...
def _default_cache_dir() -> str:
    return _env_str("SOLUS_CACHE_DIR") or DEFAULT_CACHE_DIR
...
def build_bootstrap_config_toml() -> str:
    ...
    lines = [
        "# Auto-generated by `solus config`.",
...
def load_config(config_path: str | Path | None = None) -> Config:
    ...
    security_mode = (_env_str("SOLUS_SECURITY_MODE") or str(security_data.get("mode", "trusted"))).strip().lower()
    ...
    oidc_issuer = (_env_str("SOLUS_OIDC_ISSUER") or str(security_data.get("oidc_issuer", ""))).strip()
    oidc_audience = (_env_str("SOLUS_OIDC_AUDIENCE") or str(security_data.get("oidc_audience", ""))).strip()
    oidc_require_auth_env = _env_bool("SOLUS_OIDC_REQUIRE_AUTH")
    oidc_require_auth = (
    ...
    webhook_secret = (_env_str("SOLUS_WEBHOOK_SECRET") or str(security_data.get("webhook_secret", ""))).strip()
    webhook_rate_limit_raw = _env_str("SOLUS_WEBHOOK_RATE_LIMIT") or security_data.get("webhook_rate_limit", 60)
    try:
    ...
    audit_enabled_env = _env_bool("SOLUS_AUDIT_ENABLED")
    audit_enabled = audit_enabled_env if audit_enabled_env is not None else bool(audit_data.get("enabled", True))
    audit_syslog_addr = (
        _env_str("SOLUS_AUDIT_SYSLOG_ADDR") or _env_str("AUDIT_SYSLOG_ADDR") or str(audit_data.get("syslog_addr", ""))
    ).strip()
