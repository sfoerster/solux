...
def cmd_init(args: argparse.Namespace | None = None) -> int:
    ...
    print(f"  1. Try it:  {bold('solus https://example.com/any-article')}")
    print(f"  2. Edit:    {dim(str(scaffold_path))}")
    print(f"  3. Explore: {bold('solus examples')}")
    print(f"  4. Audio:   {bold('solus doctor --workflow audio_summary')}")
