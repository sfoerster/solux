...
def cmd_worker_start(args: argparse.Namespace) -> int:
    ...
    if reason == "start-failed":
        print(
            ...
            "Run `python -m solus.cli worker start --_run-loop` to inspect startup errors.",
            file=sys.stderr,
        )
        return 1
    print("Worker appears to be running (lock held) but no pid file is available.")
    print("If this is stale, stop old worker processes and run `solus worker stop` again.")
    return 1
...
def cmd_worker_stop(args: argparse.Namespace) -> int:
    ...
    if pid is None:
        ...
        if lock_held:
            ...
            print("Stop the worker process manually, then run `solus worker status`.")
            return 1
    ...
    print(
        f"Worker pid {pid} did not stop within {args.timeout:.1f}s. Run `solus worker stop --force` if needed.",
        file=sys.stderr,
    )
    return 1
