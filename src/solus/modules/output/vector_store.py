...
def handle(ctx: Context, step: Step) -> Context:
    ...
    collection_name = str(step.config.get("collection", "solus"))
    db_path = Path(str(step.config.get("db_path", "~/.local/share/solus/chroma"))).expanduser()
...
MODULE = ModuleSpec(
    ...
    config_schema=(
        ConfigField(name="collection", description="ChromaDB collection name", default="solus"),
        ConfigField(
            name="db_path", description="Path to ChromaDB storage directory", default="~/.local/share/solus/chroma"
        ),
