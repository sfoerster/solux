...
def handle(ctx: Context, step: Step) -> Context:
    try:
        ...
        raise RuntimeError(
            "input.parse_pdf requires pypdf. Install with: pip install 'solus[pdf]' or pip install pypdf>=4.0"
        ) from exc
...
MODULE = ModuleSpec(
    ...
    version="0.1.0",
    category="input",
    description="Extract text from a PDF file using pypdf (install: pip install 'solus[pdf]').",
    handler=handle,
    dependencies=(Dependency(name="pypdf", kind="binary", hint="pip install 'solus[pdf]'"),),
    config_schema=(
