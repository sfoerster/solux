"""CLI commands for managing triggers (``solux triggers``)."""
...
def cmd_triggers_list() -> int:
    ...
    if not triggers:
        ...
        print("Tip: create a YAML file there or use `solux triggers examples` for templates.")
    for t in triggers:
...
def cmd_triggers_delete(name: str, yes: bool = False) -> int:
    ...
    print("Note: restart the worker for this change to take effect (`solux worker stop && solux worker start`).")
    return 0
