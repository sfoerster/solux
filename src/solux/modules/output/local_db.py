...
def handle(ctx: Context, step: Step) -> Context:
    db_path = Path(str(step.config.get("db_path", "~/.local/share/solux/records.db"))).expanduser()
...
MODULE = ModuleSpec(
    ...
    config_schema=(
        ConfigField(
            name="db_path", description="Path to SQLite database file", default="~/.local/share/solux/records.db"
        ),
