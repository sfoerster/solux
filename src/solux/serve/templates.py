...
def build_workflows_page(workflows: list, errors: list[str]) -> str:
    ...
    return f"""<!DOCTYPE html><html><head><title>Workflows — Solux</title>
...
<form method="POST" action="/workflow/delete" style="display:inline"
  ...
  <button type="submit" class="danger">Delete workflow</button>
    ...
    return f"""<!DOCTYPE html><html><head><title>{"New Workflow" if name == "new" else "Edit: " + esc_name} — Solux</title>
...
def build_modules_page(specs: list) -> str:
    ...
    return f"""<!DOCTYPE html><html><head><title>Modules — Solux</title>
<style>body{{font-family:system-ui;background:#0f0f23;color:#e2e8f0;margin:0}}
...
def build_history_page(jobs: list[dict], *, page: int = 1, total_pages: int = 1) -> str:
  ...
  <form method="POST" action="/bulk-clear-dead">
    ...
    return f"""<!DOCTYPE html><html><head><title>History — Solux</title>
<style>body{{font-family:system-ui;background:#0f0f23;color:#e2e8f0;margin:0}}
table{{border-collapse:collapse;width:100%;font-size:13px}}
...
def build_triggers_page(triggers: list, errors: list[str]) -> str:
    ...
    return f"""<!DOCTYPE html><html><head><title>Triggers — Solux</title>
{_DARK_STYLE}</head><body>
{_build_nav("triggers")}
<div style="padding:16px">
<h1>Triggers</h1>
...
<form method="POST" action="/trigger/delete" style="display:inline"
  ...
  <button type="submit" class="danger">Delete trigger</button>
    ...
    return f"""<!DOCTYPE html><html><head><title>{"New Trigger" if name == "new" else "Edit: " + esc_name} — Solux</title>
{_DARK_STYLE}</head><body>
{_build_nav("triggers")}
<div style="padding:16px">
...
def build_config_editor_page(
    ...
    *,
    ...
    saved_html = (
        "<div class='success'>Saved. Restart <code>solux serve</code> for changes to take effect.</div>"
        if saved
        else ""
    )
    err_html = f"<div class='error'>{html.escape(error)}</div>" if error else ""
    return f"""<!DOCTYPE html><html><head><title>Config — Solux</title>
{_DARK_STYLE}</head><body>
{_build_nav("config")}
<div style="padding:16px">
<h1>Configuration</h1>
<p style="color:#a0aec0;font-size:13px">File: <code>{esc_path}</code>
  &nbsp;·&nbsp; CLI: <code>solux config</code> · <code>solux config edit</code></p>
<div class='notice'><strong>Config changes require a server restart</strong> to take effect.
  Stop and re-run <code>solux serve</code> after saving.</div>
{saved_html}{err_html}
...
<div class='card'>
  ...
  <div class='actions'>
    ...
    return f"""<!DOCTYPE html><html><head><title>Examples — Solux</title>
{_DARK_STYLE}</head><body>
{_build_nav("examples")}
<div style="padding:16px">
<h1>Examples &amp; Templates</h1>
<p style="color:#a0aec0">Click <em>Use as template</em> to pre-fill the editor.
  CLI equivalents: <code>solux workflows examples</code> · <code>solux triggers examples</code></p>
