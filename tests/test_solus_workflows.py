...
def test_solus_parse_args_shorthand_uses_run_and_config_default_workflow() -> None:
...
def test_solus_parse_args_workflows_show() -> None:
...
def test_load_config_reads_default_solus_config(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    config_path = tmp_path / ".config" / "solus" / "config.toml"
...
""".strip(),
    ...
    assert config.paths.cache_dir == (tmp_path / ".local" / "share" / "solus").resolve()
