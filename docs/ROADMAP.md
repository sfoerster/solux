...
**Goal:** Make Solus genuinely useful on a cold install — from `pip install` to running a real workflow in under 60 seconds.

### 1.1 `solus init` Command

- [x] Add `solus init` command that does everything a first-time user needs in one shot:
  1. Create `~/.config/solus/config.toml` with auto-discovered defaults (Ollama URL, model, binary paths)
  ...
  5. Run `solus doctor --workflow webpage_summary` to validate the light path
  6. Print a "next steps" block with the exact command to run their first workflow
- [x] `solus init` should be idempotent — safe to re-run without clobbering existing config
- [x] Ensure `solus init` completes in under 10 seconds (no network calls beyond an Ollama ping)
...
- [x] Make the bare `solus <URL>` shorthand use `webpage_summary` so that `solus https://example.com` just works after init
...
- [x] Improve `solus doctor` output for first-time users: when a check fails, print the **exact next command** to run, not just "not found"
...
- [x] `solus run --dry-run` output polish — show the full execution plan visually (step names, types, arrows between them) so users understand what will happen before it runs
- [x] Add `solus examples` shorthand (alias for `solus workflows examples`)
- [x] Improve `solus run` output — show step progress inline (step 1/3, 2/3, 3/3) with timing, not just a final dump
...
- [x] Write an end-to-end smoke test script that times the full path: `pip install -e .` → `solus init` → `solus run --workflow webpage_summary <URL>` — assert it completes in under 60 seconds on a machine with Ollama already running
- [x] Test on a clean environment (fresh venv, no prior config) to catch any assumptions
- [x] Document the minimum viable setup: Python 3.11+, Ollama with any model, `pip install solus`, `solus init`, go (see `docs/QUICK_START.md`)

---

## Phase 2 — Public Launch

**Goal:** Ship Solus to GitHub. Package it for easy installation.

### 2.1 License & Packaging

- [x] Choose license (Apache 2.0)
- [x] Replace the current proprietary license file
- [ ] Publish to PyPI: `pip install solus` and `pipx install solus`
...
- [x] Add CHANGELOG.md
- [x] Clean up any Vinsium-specific references in user-facing docs (the `output.vinsium_node` module stays, but framing shifts to "remote Solus node")

### 2.3 Branding & Landing Page

- [ ] Solus logo and visual identity
...
**Goal:** Build a module ecosystem. Establish Solus as the default tool for local AI automation.
...
- [ ] **`solus dev`** — watch a workflow YAML file, re-run on save (hot dev loop)
- [ ] **Template variables** — workflow-level `vars:` block for reusable values across steps
- [ ] **Module scaffold CLI** — `solus modules new my_module` generates a skeleton `.py` file with correct `ModuleSpec` boilerplate
...
- [ ] `solus modules install <github-url>` — download a module from a Git repo into `modules.d/`
...
- [ ] **Multi-node workflow coordination** — orchestrate workflows across Solus instances (building on `output.vinsium_node`)
- [ ] **Centralized audit aggregation** — collect audit logs from multiple Solus nodes into a single dashboard
- [ ] **Fleet management API** — deploy workflows and config updates to multiple Solus instances

---

## Non-Goals

Things Solus will **not** become:

- **A cloud service.** Solus is local-first.
...
- **A general-purpose task runner.** Solus is specifically for AI/LLM workflows with local inference. Generic CI/CD or task orchestration (Make, Airflow, Prefect) is out of scope.
- **A replacement for Ollama/whisper.cpp/ffmpeg.** Solus orchestrates these tools; it doesn't reimplement them.
